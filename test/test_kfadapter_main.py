...
class testKfadapterApi(TestCase):
    ...
    def test_get_all_runs(self, mock_get_kf_list_runs):
        ...
        for i, run in enumerate(runs) :
            ...
            run.pipeline_spec = {
                ...
                "pipeline_name": f"pipeline-name{i}"
            }
        ...
        self.assertEqual(response.content_type, "application/json")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.get_data(), b'{"run-name0":{"experiment_id":"experiment-id0","pipeline_id":"pipeline-id0","pipeline_name":"pipeline-name0","run_description":"description","run_id":"runid","run_state":"state"},"run-name1":{"experiment_id":"experiment-id1","pipeline_id":"pipeline-id1","pipeline_name":"pipeline-name1","run_description":"description","run_id":"runid","run_state":"state"}}\n')
    ...
    def test_get_run(self, mock_get_kf_run):
        ...
        self.assertEqual(response.get_json()["run_state"], run.state)
